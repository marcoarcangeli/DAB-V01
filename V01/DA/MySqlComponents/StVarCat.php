...
class StVarCat
{
    ...
    // object properties
    /*  
StVarCat(IdStVarCat, Nam, Descr, timestamp, IdUsr)
    */
    public $IdStVarCat;
    public $IdStVarCatPar;
    public $Nam;
    public $Descr;
    ...
    public function selectAll()
    {
        try {
            ...
            SELECT
                p.IdStVarCat,
                p.IdStVarCatPar,
                pp.Nam AS StVarCatParNam,
                p.Nam,
                p.Descr
            FROM
                StVarCat p
                LEFT OUTER JOIN StVarCat pp ON p.IdStVarCatPar = pp.IdStVarCat
            ORDER BY
                pp.Nam,p.Nam
            ";
    ...
    */
    ...
    public function insert()
    {
        try {
            ...
            $query = "INSERT INTO  StVarCat
                        (IdStVarCatPar, Nam, Descr)
                  VALUES
                        (
                         " . VN($this->IdStVarCatPar) . ",
                         " . VN($this->Nam) . ",
                         " . VN($this->Descr) . "
                         )";
            // throw new exception($query."\n");
            ...
            // execute query
            if ($Stmt) {
                $this->IdStVarCat = $this->conn->insert_id;
                return true;
            }
            return false;
        } catch (Exception $e) {
    ...
    public function update()
    {
        try {
            ...
            $query = "UPDATE
                    StVarCat
                SET
                    IdStVarCatPar="   .VN($this->IdStVarCatPar) . ",
                    Nam="           .VN($this->Nam) . ",
                    Descr="         .VN($this->Descr) . "
                WHERE
    ...
    public function delete()
    {
        try {
            ...
            $query = "DELETE from StVarCat 
                WHERE IdStVarCat = '" . $this->IdStVarCat . "'";
            //throw new exception($query."\n");
            // prepare query statement
            $Stmt = mysqli_query($this->conn, $query);
            //throw new exception("righe trovate ".$Stmt->num_rows."\n");
