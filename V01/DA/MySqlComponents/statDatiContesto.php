...
class statDatiCntx 
{
    ...
    // object properties
    /*  
statDatiCntx(idStatDatiCntx, IdCntx, note, nDimensioni, nMisure, proiezioneEvnt
    */
    ...
    public function insert()
    {
        try {
            ...
            //    return false;
            //}
            // statDatiCntx(idStatDatiCntx, IdCntx, note, nDimensioni
            // query to insert record
            $query = "INSERT INTO  statDatiCntx
                        (IdCntx, note, nDimensioni, nMisure, proiezioneEvnt)
                  VALUES
                        (" . VN($this->IdCntx) . ",
                         " . VN($this->note) . ",
                         " . VN($this->nDimensioni) . ",
            ...
            // execute query
            if ($Stmt) {
                $this->idStatDatiCntx = $this->conn->insert_id;
                return true;
            }
            return false;
        } catch (Exception $e) {
    ...
    public function delete()
    {
        try {
            ...
            $query = "DELETE from statDatiCntx 
                WHERE idStatDatiCntx = '" . $this->idStatDatiCntx . "'";
            //throw new exception($query."\n");
            // prepare query statement
            $Stmt = mysqli_query($this->conn, $query);
            //throw new exception("righe trovate ".$Stmt->num_rows."\n");
