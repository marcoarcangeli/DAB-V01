...
class OpDatCat
{
    ...
    // object properties
    /*  
OpDatCat(IdOpDatCat, Nam, Descr, timestamp, IdUsr)
    */
    public $IdOpDatCat;
    public $IdOpDatCatPar;
    public $Nam;
    public $Descr;
    ...
    */
    ...
    public function insert()
    {
        try {
            ...
            $query = "INSERT INTO  OpDatCat
                        (IdOpDatCatPar, Nam, Descr)
                  VALUES
                        (
                         " . VN($this->IdOpDatCatPar) . ",
                         " . VN($this->Nam) . ",
                         " . VN($this->Descr) . "
                         )";
            ...
            // execute query
            if ($Stmt) {
                $this->IdOpDatCat = $this->conn->insert_id;
                return true;
            }
            return false;
        } catch (Exception $e) {
    ...
    public function update()
    {
        try {
            ...
            $query = "UPDATE
                    OpDatCat
                SET
                    IdOpDatCatPar="   .VN($this->IdOpDatCatPar) . ",
                    Nam="           .VN($this->Nam) . ",
                    Descr="         .VN($this->Descr) . "
                WHERE
    ...
    public function delete()
    {
        try {
            ...
            $query = "DELETE from OpDatCat 
                WHERE IdOpDatCat = '" . $this->IdOpDatCat . "'";
            //throw new exception($query."\n");
            // prepare query statement
            $Stmt = mysqli_query($this->conn, $query);
            //throw new exception("righe trovate ".$Stmt->num_rows."\n");
