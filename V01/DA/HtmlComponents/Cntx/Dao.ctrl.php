...
include_once($BFD.'DA/mySqlComponents/Cntx.php');
...
use DA\mySqlComponents\Cntx as DAO;

class DaoCtrl
{
    // common params with deafaults;
    public string $CompulsoryParamNams = 'IdCntx';
    public string $OptionalParamNams = ''; //'SearchIds';
    // public string $ParamSource = 'PSV';
    // public string $ParamSource2 = 'ASV';
    // public string $SaveParamNams = "IdCntx,IdCntxState,IdCntxCat,Nam,Descr,fileRefProc,CatTag";
    ...
    {
        $CompulsoryParamNams = $this->CompulsoryParamNams; //"IdCntx";
        try {
            // get Database connection
            $Db = new DB();
            $Db = $Db->getConnection();
        ...
        try {
            ...
            if (
                ...
                if ($Dao->insert()) {
                    ...
                    $_SESSION["PSV"][$CompulsoryParamNams]=$Params[$CompulsoryParamNams];
                    $_SESSION["PSV"]["FolderNam"]="Cntx_" . $Params[$CompulsoryParamNams];
                    // aggiorna FS MakeFolder(FolderPathNam)
                    $_SESSION["PSV"]["FolderNam"] = $FolderNam = "Cntx_" . $Params[$CompulsoryParamNams];
                    $FolderPathNam = $_SESSION["PrjAbsPath"] . $_SESSION["PSV"]["PrjFolderNam"] . "/" . $FolderNam;
    ...
    public function FileTlistFs(array $Params)
    {
        ...
        try {
            ...
            // get manager object
            $Dir  = $_SESSION["PrjAbsPath"].$_SESSION["PSV"]["PrjFolderNam"].'/'.$_SESSION["PSV"]["CntxFolderNam"].'/';
