...
use DA\mySqlComponents\Evnt as DAO;

class DaoCtrl
{
    // common params with deafaults;
    public string $CompulsoryParamNams = 'IdEvnt';
    public string $OptionalParamNams = ''; //'SearchIds';
    // public string $ParamSource = 'PSV';
    // public string $ParamSource2 = 'ASV';
    // public string $SaveParamNams = "IdEvnt,IdEvntState,IdEvntCat,Nam,Descr,fileRefProc,CatTag";
    ...
    {
        $CompulsoryParamNams = $this->CompulsoryParamNams; //$CompulsoryParamNams;
        try {
            // get Database connection
            $Db = new DB();
            $Db = $Db->getConnection();
        ...
        try {
            ...
            if (
                ...
                if ($Dao->insert()) {
                    ...
                    $_SESSION["PSV"]["EvntFolderNam"]="Evnt_" . $Dao->{$CompulsoryParamNams};
                    $_SESSION["PSV"]["FileNamRepoDat"]=pathinfo($Evnt->fileRefRepoDat)['filename'];
                    if($_SESSION["Debug"]>=2){ LM::LogMessage("DEBUG","Evnt->insert: ".$Dao->{$CompulsoryParamNams}); }
                    // update FS
                    $_SESSION["EvntFolderNam"] = $EvntFolderNam = "Evnt_" . $Dao->{$CompulsoryParamNams};
                    $_SESSION["EvntFolderNam"] = $EvntFolderNam = $_SESSION["EvntFolderPrfx"] . $Dao->{$CompulsoryParamNams};
                    $EvntFolderPathNam = $_SESSION["PrjAbsPath"] . $_SESSION["PSV"]["PrjFolderNam"] . "/" . $EvntFolderNam;
                    // throw new exception("EvntFolderPathNam: ".$EvntFolderPathNam."\n");
                    if($_SESSION["Debug"]>=2){ LM::LogMessage("DEBUG","EvntFolderPathNam: ".$EvntFolderPathNam); }
