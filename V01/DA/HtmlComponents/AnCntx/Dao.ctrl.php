...
include_once($BFD.'DA/mySqlComponents/AnCntx.php');
...
use DA\mySqlComponents\AnCntx as DAO;

class DaoCtrl
{
    // common params with deafaults;
    public string $CompulsoryParamNams = 'IdAnCntx';
    public string $OptionalParamNams = ''; //'SearchIds';
    // public string $ParamSource = 'PSV';
    // public string $ParamSource2 = 'ASV';
    // public string $SaveParamNams = "IdAnCntx,IdAnCntxState,IdAnCntxCat,Nam,Descr,fileRefProc,CatTag";
    ...
    {
        $CompulsoryParamNams = $this->CompulsoryParamNams; //"IdAnCntx";
        try {
            // get Database connection
            $Db = new DB();
            $Db = $Db->getConnection();
    ...
    public function SaveDb(array $Params)
    {
        ...
        try {
            ...
            if($TransactionState){
                ...
                $_SESSION["ASV"][$CompulsoryParamNams]=$Dao->{$CompulsoryParamNams};
            }else{
                $Db->rollback();
                $Result_arr = array(
                    "State" => false,
    ...
    public function FileTlistFs(array $Params)
    {
        ...
        try {
            ...
            // $AnCntx->idPrj = $_POST['idPrj'];
            // get manager object
            $Dir  = $_SESSION["PrjAbsPath"].$_SESSION["PSV"]["PrjFolderNam"].'/'.$_SESSION["PSV"]["AnFolderNam"].'/';
