...
da.ProfileFeatureAuthRead = {
    ...
    Refresh: function() {
        try {
            if (da.ProfileFeatureAuthRead.IdProfileFeatureAuth && da.ProfileFeatureAuthRead.IdProfileFeatureAuth != '') {
                return $.ajax({
                    type: "GET",
                    url: "DA/HtmlComponents/ProfileFeatureAuth/read.proxy.php?IdProfileFeatureAuth=" + da.ProfileFeatureAuthRead.IdProfileFeatureAuth,
                    dataType: "json",
    ...
    Save: function() {
        try {
            // alert("#btnSave");
            if (da.verifyCompulsoryFields(da.ProfileFeatureAuthRead.CompulsoryFields, da.ProfileFeatureAuthRead
                    .PanelTag)) {
                return $.ajax({
                    type: "POST",
                    ...
                    success: function(result) {
                        ...
                        if (result["State"]) {
                            ...
                            if (da.ProfileFeatureAuthRead.ParentObj) {
                                ...
                                da.RefreshObj(da.ProfileFeatureAuthRead.ParentObj, da.ProfileFeatureAuthRead.ParentObjType, "Refresh",null);
                            }
                            da.UsrMsgShow(da.ProfileFeatureAuthRead.SuccessMsg, "Info");
                        } else {
                            da.UsrMsgShow(da.ProfileFeatureAuthRead.FailMsg, "Info");
                ...
                $.each(response.data, function(index, item) { // Iterates through a collection
                    $("#ProfileFeatureAuth_IdFeature")
                        .append( // Append an object to the inside of the select box
                            $("<option></option>") // Yes you can do this.
                            .text(item.FeatureCatNam+ "/" + item.Nam + " (" + item.IdFeature + ")")
                            .val(item.IdFeature)
                        );
                });
