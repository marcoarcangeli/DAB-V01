...
include_once($BFD.'DA/mySqlComponents/AlgParamType.php');
...
use DA\mySqlComponents\AlgParamType as DAO;

class DaoCtrl
{
    // common params with deafaults;
    public string $CompulsoryParamNams = 'IdAlgParamType';
    public string $OptionalParamNams = ''; //'SearchIds';
    // public string $ParamSource = 'PSV';
    // public string $ParamSource2 = 'ASV';
    // public string $SaveParamNams = "IdAlgParamType,IdAlgParamTypeState,IdAlgParamTypeCat,Nam,Descr,fileRefProc,CatTag";
    ...
    {
        $CompulsoryParamNams = $this->CompulsoryParamNams; //"IdAlgParamType";
        try {
            // get Database connection
            $Db = new DB();
            $Db = $Db->getConnection();
        ...
        try {
            ...
            // ) {
                ...
                if($Dao->{$CompulsoryParamNams} == ""){
                    $Results = $Dao->selectParamTypeAlg();
                }else{
